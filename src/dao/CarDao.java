...
public class CarDao {
    ...
    public Car getById(int id){
        Car obj = null;
        String query = "SELECT * FROM public.car WHERE id = ?";
        try {
    ...
    public ArrayList<Car> findAll(){return this.selectByQuery("SELECT * FROM public.car ORDER BY ID ASC");
    }
    ...
    public boolean delete(int carId){
        String query = "DELETE FROM public.car WHERE id = ?";
        try{
            PreparedStatement pr = con.prepareStatement(query);
            pr.setInt(1,carId);
            return pr.executeUpdate() !=-1;
